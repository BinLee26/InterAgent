...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        self.enable_hist_obs = cfg["env"]["histObs"]["EnableHistObs"]
        if self.enable_hist_obs:
            self.hist_obs_steps = cfg["env"]["histObs"]["histObsStep"]
        ...
        print("Numobs CHECK:", self.num_obs)
        # allocate buffers
        if not self.enable_hist_obs:
            ...
            self.obs_buf = torch.zeros(
                (self.num_envs, self.num_agents, self.hist_obs_steps, self.num_obs), device=self.device, dtype=torch.float)
            self.curr_obs_buf = self.obs_buf[:,:,0]
            self.hist_obs_buf = self.obs_buf[:,:,1:]
        
        #print("obs_buf_size:" , self.obs_buf.size())
        ...
        self.progress_buf = torch.zeros(
            self.num_envs, self.num_agents, device=self.device, dtype=torch.long)
        #print("buf CHECK:", self.progress_buf[0,:])
        ...
        self.record_frames = cfg["record_frames"]
        self.record_frames_dir = cfg["record_frames_dir"]
    ...
    def render(self, steps):
        if self.viewer:
            ...
            if self.record_frames:
                ...
                self.gym.write_viewer_image_to_file(self.viewer, join(self.record_frames_dir, f"frame_{steps}.png"))
