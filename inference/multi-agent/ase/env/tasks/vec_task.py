...
class VecTaskPython(VecTask):
    ...
    def get_state(self):
        ...
        return torch.clamp(self.task.states_buf.reshape(num_envs*num_agents, -1), -self.clip_obs, self.clip_obs).to(self.rl_device)
    ...
    def step(self, actions):
        ...
        if not enable_hist_obs:
            obs_buf = self.task.obs_buf#.reshape(num_envs*num_agents, -1)
        else:
            obs_step = self.task.obs_buf.shape[-2]
            obs_buf = self.task.obs_buf#.reshape(num_envs*num_agents, obs_step*obs_dim)
        
        return torch.clamp(obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.view(num_envs*num_agents).to(self.rl_device), self.task.reset_buf.view(num_envs*num_agents).to(self.rl_device), self.task.extras
    ...
    def reset(self):
        ...
        return torch.clamp(self.task.obs_buf.view(num_envs*num_agents, -1), -self.clip_obs, self.clip_obs).to(self.rl_device)
