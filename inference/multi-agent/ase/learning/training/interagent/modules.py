...
class TransformerForDiffusion(nn.Module):
    def __init__(self,
        ...
        self.cond_pos_emb_recent4_ig_seq = nn.Parameter(torch.zeros(1, 4*15*15, 36))#nn.Parameter(torch.zeros(1, 4*225, n_emb_ig_seq))
        self.cond_pos_emb_faraway12_ig_seq = nn.Parameter(torch.zeros(1, 12*15*15, 36))#nn.Parameter(torch.zeros(1, 12*225, n_emb_ig_seq))
    ...
    def forward(self,
        ...
        exteroception_a_recent4_ig_raw = exteroception_a_recent4.reshape(-1,4,225,3)
        exteroception_b_recent4_ig_raw = exteroception_b_recent4.reshape(-1,4,225,3)

        exteroception_a_recent4_ig_seq = exteroception_a_recent4_ig_raw.reshape(-1,4*225,3)
        exteroception_b_recent4_ig_seq = exteroception_b_recent4_ig_raw.reshape(-1,4*225,3)
