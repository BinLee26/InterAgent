...
from Joints2SMPL.joints2smpl import joints2smpl

default_betas = ([[-0.3890, -1.4800, -0.3150,  1.3800,  0.6160,  0.2000, -0.1090, -0.0450,
          0.0060,  0.0460]])
N_JOINTS = 22

def get_local_translation(raw_root_gt, p):
...
def retarget_motion(source_motion_path, target_motion_path):
    ...
    raw_motion = np.load(source_motion_path)
    print('+++raw',raw_motion.shape)
    print("retargeting:", source_motion_path)
    if(raw_motion.shape[0] == 0):
        print("empty! retargeting next")
        return

    retarget_data_path = "ase/poselib/data/configs/retarget_smpl_to_amp.json"
    ...
    for p in range(2):
        ...
        smpl_joints = joints2smpl(raw_motion[:,p,:])
        print('+++++smpl',smpl_joints.shape)
        #trans = torch.from_numpy(smpl_joints[:,:3])
        #root_orient = torch.from_numpy(smpl_joints[:,3:6])
        
        # root oritent
        Rh = smpl_joints[:,3:6].copy()
        R_x = R.from_euler('x', 90, degrees=True).as_matrix()
        Rh_rot_mat = R.from_rotvec(Rh).as_matrix()
        Rh_new_rot_mat = R_x @ Rh_rot_mat
        Rh_new = R.from_matrix(Rh_new_rot_mat).as_rotvec()
        #root_orient = torch.from_numpy(Rh_new)
        R_x = R.from_euler('z', 90, degrees=True).as_matrix()
        Rh_rot_mat = R.from_rotvec(Rh_new).as_matrix()
        Rh_new_rot_mat = R_x @ Rh_rot_mat
        Rh_new_new = R.from_matrix(Rh_new_rot_mat).as_rotvec()
        root_orient = torch.from_numpy(Rh_new_new)
        
        # root translation
        Th = smpl_joints[:,:3].copy()
        Th_new = Th[:, [2, 0, 1]]
        trans = torch.from_numpy(Th_new)
        
        pose_body = torch.from_numpy(smpl_joints[:,6:])
        
        smpl_local_translation = get_local_translation(trans, p)
        ...
        num_frame = root_orient.size(0)
        #*********************88
        #raw_joints_local_rot_6d = torch.from_numpy(raw_joints_local_rot_6d)
        res = torch.zeros((num_frame, 6))
        rot_vecs = torch.cat((root_orient, pose_body), dim=1)
        assert rot_vecs.shape == (num_frame, 72) 
        rot_vecs = rot_vecs.reshape(-1,24,3)
        ...
        source_motion = SkeletonMotion.from_skeleton_state(smpl_skeleton_state, 30)
...
def main():
    ...
    target_path = 'ase/data/motions/intergen_amp_testt/'
