...
def retarget_motion(source_motion_path, target_motion_path):
    ...
    # parse data from retarget config
        ...
        if p == 'person1':
            target_path = os.path.join(target_motion_path, 'person1.npy')
        else:
            target_path = os.path.join(target_motion_path, 'person2.npy')
        target_motion.to_file(target_path)
        #plot_skeleton_motion_interactive(target_motion)

def main():
    source_path = '../data/interhuman_mix/'
    target_path = '../data/interhuman_amp/'
    source_motion_path = source_path + r"smpl_params.pkl"
    ...
    target_motion_path = source_path + r"test.npy"
    ...
    #             target_motion_path = os.path.sep.join(root.split(os.path.sep)[3:])
    #             target_motion_path = os.path.join(target_path, target_motion_path)
