...
smpl_local_robot = LocalRobot(
    robot_cfg,
    data_dir="ase/poselib/body_model/smpl",
)

print('++++robot created')
amass_data = joblib.load("ase/data/InterGenTest/1589.pkl")
...
full_motion_dict = {}
fps = amass_data.get("mocap_framerate", 30.0)

for key_name in ['person1', 'person2']:
    print('++++++fps',fps)    
    print('++++++person',key_name)
    smpl_data_entry = amass_data[key_name]
    print('+++++keys',smpl_data_entry.keys())
    B = smpl_data_entry['pose_body'].shape[0]

    start, end = 0, 0

    pose_aa = smpl_data_entry['pose_body'].copy()[start:]
    root_orient = smpl_data_entry['root_orient'].copy()[start:]
    ...
    pose_aa = np.concatenate([root_orient[:,:3],pose_aa[:, :63], np.zeros((batch_size, 6))], axis=1)
    ...
    for idx in range(num):
        ...
        smpl_local_robot.write_xml("ase/data/assets/mjcf/smpl_humanoid_1.xml")
...
joblib.dump(full_motion_dict, "ase/poselib/")
