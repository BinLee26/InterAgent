...
from datasets import InterDPDataset
from models import *
import copy
from datasets.evaluator_models import InterCLIP, MultiInterCLIP
...
def get_dataset_motion_loader(opt, batch_size,replication):
    opt = copy.deepcopy(opt)
    # Configurations of T2M dataset and KIT dataset is almost the same
    if opt.NAME == 'interdp':
        print('Loading dataset %s ...' % opt.NAME)

        dataset = InterDPDataset(opt,replication)
        print("--!!！！！！！get_dataset_motion_loader replication:",replication)
    ...
    print(f'{opt.MODE} Dataset Loading Completed!!!')
...
def build_models(cfg):
    ...
    checkpoint = torch.load(pjoin("/Interhuman/epoch=999-step=21000.ckpt"),map_location="cpu")
